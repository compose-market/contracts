...
contract Compose is Script {
    ...
    bytes32 constant REPUTATION_SALT = keccak256("compose:reputation:v1:2026");
    bytes32 constant VALIDATION_SALT = keccak256("compose:validation:v1:2026");
    ...
    function run() external {
        ...
        reputation = Reputation(
            _deployDeterministic(
                "Reputation",
                REPUTATION_SALT,
                abi.encodePacked(type(Reputation).creationCode, abi.encode(address(agentFactory)))
            )
        );
        ...
        validation = Validation(
            _deployDeterministic(
                "Validation",
                VALIDATION_SALT,
                abi.encodePacked(type(Validation).creationCode, abi.encode(address(agentFactory)))
            )
        );
        ...
        utils = Utils(
            _deployDeterministic(
                ...
                abi.encodePacked(
                    ...
                    abi.encode(address(agentManager), address(agentFactory), address(workflow))
                )
            )
        );
